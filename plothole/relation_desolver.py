...
def desolve_parent_by_path(path, return_path_only=False):
    ...
    parent_folder = pathlib.Path(pathlib.Path(path).parent).parent
    log.log_var(None, currentframe(), ("parent_folder", parent_folder))
    
    parent_ptype = pt.PARENT_PLOTHOLE_TYPE.get(pathlib.Path(path).suffix.split(".")[-1])
...
def desolve_children_by_path(path, return_path_only=False):
    ...
    child_ptype = pt.CHILD_PLOTHOLE_TYPE.get(pathlib.Path(path).suffix.split(".")[-1])
    log.log_var(None, currentframe(), ("child_ptype", child_ptype))
    
    children_se = None
    
    if child_ptype is not None:
        children_se = hlp.get_all(parent_folder, child_ptype, as_dict=True)
